...
def relaxation_procedure():
    ...
    while(1):
        ...
        for i in xrange(len(dataset)):
            sumEval = 0
            # evaluating each of the points for correctly or incorrectly
            # classified determination
            for j in xrange(3):
                sumEval += dataset[i][j]*weight[j]

            if sumEval < 0.1:
                # 0.9 is the value of the 'b' used in the equation above,
                # for further values of 'b' like 1/2/.. the Iterations
                # dont stop and here also it stops at close to 50k iterations.
                # with 'b=0.2' no. of iterations reduces to 5200 and
                # it further reduces to 3.5k on 'b=0.1'.
                missClass.append(dataset[i])
            semiAns = [0, 0, 0]
            for i in xrange(len(missClass)):
                numer = float((1-evaluate(weight, missClass[i])) /
                              square_values(missClass[i]))
                for j in xrange(3):
